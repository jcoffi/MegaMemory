...
For decisions / experiments / results: \`open\` (proposed — not a correctness claim) · \`validated\` (confirmed for a stated scope, by explicit evidence — never assumed) · \`refuted\` · \`superseded\` · \`abandoned\`. Omit for descriptive concepts that mirror code — UNLESS something cites the concept as evidence (an incoming \`informed_by\`): then set \`validated\` once you confirm it still matches the code, stating the scope. A cited concept left status-less sits on the frontier permanently and makes every \`validated\` decision resting on it trip the \`validated_rests_on_unvalidated\` hygiene flag. Legacy NULL means unknown, not validated. Prefer a status transition over deleting an epistemic record.
